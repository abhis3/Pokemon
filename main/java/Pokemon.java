...
public abstract class Pokemon {
    ...
    */
    ...
    public void move() {
        ...
        delta = (rand.nextInt((maxRand - minRand) + 1) + minRand) * speed;

        if ((xPos + delta > 0)
            && (xPos + delta < (bounds.getWidth() - image.getIconWidth()))) {
            xPos = xPos + delta;
        } else {
            xPos = xPos + ((-1) * delta);
        }
        ...
        if ((yPos + delta > 0)
            && (yPos + delta < (bounds.getHeight() - image.getIconHeight()))) {
            yPos = yPos + delta;
        } else {
            yPos = yPos + ((-1) * delta);
        }
    ...
    public boolean collidesWithPokemon(Pokemon other) {
        ...
        if ((((xPos + image.getIconWidth()) >= other.getXPos())
            && ((xPos <= (other.getXPos() + other.image.getIconWidth()))))) {

            if ((((yPos + image.getIconHeight()) >= other.getYPos())
                    && ((yPos <= (other.getYPos()
                        + other.image.getIconHeight()))))) {
                return true;
            }
        }
        return false;
